const { registerValidator, ErrorResponse, loginValidator } = require("../lib/auth");
...
const register = async (req, res, next) => {
  ...
  try {
    ...
    if(retVal === null) return res.status(400).json(ErrorResponse({ message: 'Failed to create a new user' }));
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    if(retVal === null) return res.status(400).json(ErrorResponse({ message: 'Failed to login' }));
...
const getUser = async (req, res, next) => {
  try {
    ...
    if(retVal === null) return res.status(400).json(ErrorResponse({ message: 'Failed to get user' }));
    return res.status(200).json(retVal);
